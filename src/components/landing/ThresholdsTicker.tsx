import { formattedTeamsFetchedAt, leagueSeason, leagueThresholds } from "@/lib/data-meta";
...
const ROWS = [
  ...
  { label: "Luxury Tax", value: leagueThresholds.luxuryTax },
    ...
    <div className="rounded-2xl border border-(--color-border) bg-(--color-surface)/60 overflow-hidden">
      <div className="flex items-center gap-1.5 px-4 pt-3 pb-1.5">
        ...
        <p className="text-[10px] tracking-[0.14em] uppercase text-(--color-text-muted)">
          {leagueSeason} League Thresholds — Updated {formattedTeamsFetchedAt}
        </p>
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-5 divide-x divide-y sm:divide-y-0 divide-(--color-border)">
        {ROWS.map((row) => (
